$(document).ready(function () {
  ...
  $("body").on(
    "click",
    "a[data-action=delete-confirmation]:not(table a)",
    linkDeleteConfirmationHandler
  );
  ...
  var addCommentLink = function (event) {
    ...
    if (clipboardData.indexOf("/#comment") > -1) {
      ...
      var newClipboardData =
        "#[" + taskId + "-" + commentId + "] (" + AppLanugage.comment + ") ";
...
(function ($) {
  $.fn.appForm = function (options) {
              ...
              if (result.success) {
                ...
                if (!settings.isModal) {
                  $(form)
                    ...
                    .each(function () {
                      if (
                        $(this).attr("data-rich-text-editor") != undefined &&
                        $(this).attr(
                          "data-keep-rich-text-editor-after-submit"
                        ) == undefined
                      ) {
    ...
    function validateForm(form, customSubmit) {
      ...
      $.validator.addMethod(
        ...
        function (value, element, params) {
          ...
          if (
            typeof $(element).attr("data-rule-required") === "undefined" &&
            !value
          ) {
            return true;
          }
    ...
    function closeAjaxModal(success) {
      if (success) {
        $(".modal-mask").html(
          "<div class='circle-done'><i data-feather='check' stroke-width='5'></i></div>"
        );
        setTimeout(function () {
...
var getWeekRange = function (date) {
  ...
  if (diff < 7) {
    range.firstDateOfWeek = moment(date)
      .subtract(diff, "days")
      .format("YYYY-MM-DD");
  } else {
  ...
  if (diff < 0) {
    range.firstDateOfWeek = moment(range.firstDateOfWeek)
      .subtract(7, "days")
      .format("YYYY-MM-DD");
  }

  range.lastDateOfWeek = moment(range.firstDateOfWeek)
    .add(6, "days")
    .format("YYYY-MM-DD");
  return range;
};
...
function getContextFilterInfo(filterId, settings) {
  ...
  if (
    ...
    filterInfo.context !== context + "_" + context_id
  ) {
...
class DefaultFilters {
  ...
  init() {
    ...
    if (
      filterId &&
      this.settings.stateSave &&
      !this.settings.ignoreSavedFilter &&
      getContextFilterInfo(filterId, this.settings)
    ) {
  ...
  prepareDefaultDateRangeFilterParams() {
    ...
    if (settings.dateRangeType === "daily") {
      settings.filterParams.start_date = moment().format(
        settings._inputDateFormat
      );
  ...
  prepareDefaultrRngeDatepickerFilterParams(settings) {
    ...
    $.each(settings.rangeDatepicker || [], function (index, datepicker) {
      if (datepicker.startDate && datepicker.startDate.value) {
        settings.filterParams[datepicker.startDate.name] =
          datepicker.startDate.value;
      }

      if (datepicker.startDate && datepicker.endDate.value) {
        settings.filterParams[datepicker.endDate.name] =
          datepicker.endDate.value;
      }
    });
    this.settings = settings;
  }
...
class BuildFilters {
  ...
  saveSelectedFilter() {
    var userId = AppHelper.userId ? AppHelper.userId : "public";
    setCookie(
      "filter_" + this.settings.smartFilterIdentity + "_" + userId,
      this.activeFilterId
    );
  }
  ...
  reloadInstance() {
    if (this.$instance.is("table")) {
      this.$instance.appTable({
        reload: true,
        filterParams: this.settings.filterParams,
      });
    } else {
      this.$instance.appFilters({
        reload: true,
        filterParams: this.settings.filterParams,
      });
    }
  ...
  prepareSmartFilterDropdown() {
    if (this.settings.smartFilterIdentity) {
      ...
      var dataPostAttrs =
        " data-post-context='" +
        it.settings.smartFilterIdentity +
        "' " +
        " data-post-instance_id='" +
        it.getInstanceId() +
        "' ";
      ...
      var actionUrl =
        AppHelper.baseUrl +
        "index.php/filters/manage_modal/" +
        it.settings.smartFilterIdentity;

      var dropdown =
        "<div class='dropdown-menu w300'>" +
        '<div class="pb10 pl10">' +
        '<a class="inline-block btn btn-default manage-filters-button" data-act="ajax-modal" data-title="' +
        AppLanugage.manageFilters +
        '" ' +
        dataPostAttrs +
        '  type="button" data-action-url="' +
        actionUrl +
        '" ><i data-feather="tool" class="icon-16 mr5"></i>' +
        AppLanugage.manageFilters +
        " </a>" +
        '<a class="inline-block btn btn-default clear-filter-button ml10 hide" href="#"><i data-feather="delete" class="icon-16 mr5"></i>' +
        AppLanugage.clear +
        "</a></div>" +
        '<input type="text" class="form-control search-filter" placeholder="' +
        AppLanugage.search +
        '">' +
      ...
      this.$instanceWrapper
        .find(it.leftFilterSectionClsss)
        .append(smartFilterDropdownDom);
      this.refreshFilterDropdown();

      this.$instanceWrapper
        .find(".smart-filter-dropdown-container")
        .on("click", ".smart-filter-item", function () {
          var data = $(this).data() || {},
            filterId = data.id;
          it.state = "new_filter";
      ...
      var $dropdown = this.$instanceWrapper.find(
      ...
      $dropdownSearch.on("keydown", function (e) {
        var $activeDropdown = it.$instanceWrapper.find(
        ...
        if (
          ...
          $activeDropdown.offset().top +
            $activeDropdown.outerHeight() -
            $listGroup.offset().top >
            $listGroup.height()
        ) {
          $listGroup.scrollTop(
            $listGroup.scrollTop() + $activeDropdown.outerHeight()
          );
        } else if (
          $activeDropdown.length &&
          $activeDropdown.offset().top - $listGroup.offset().top < 0
        ) {
          $listGroup.scrollTop(
            $listGroup.scrollTop() - $activeDropdown.outerHeight()
          );
  ...
  refreshFilterDropdown() {
    ...
    $.each(filters, function (index, filterItem) {
      ...
      options +=
        '<li><a href="#" class="dropdown-item smart-filter-item list-group-item clickable ' +
        active +
        ' "data-id="' +
        filterItem.id +
        '">';
    ...
    var smartFilterButtonText =
      '<i data-feather="filter" class="icon-16 mr5"></i>' + title;

    this.$instanceWrapper
      .find(".smart-filter-dropdown")
      .html(smartFilterButtonText);
    ...
    if (filters.length) {
      this.$instanceWrapper
        ...
        .removeClass("hide");
      this.$instanceWrapper
        .find(".show-filter-form-button")
        .find(".add-filter-text")
        .addClass("hide");
    } else {
      this.$instanceWrapper
        .find(".smart-filter-dropdown-container")
  ...
  prepareSurchOption() {
    ...
    if (settings.search && settings.search.show !== false) {
      var searchDom =
        '<div class="filter-item-box">' +
        '<input type="search" class="custom-filter-search" name="' +
        settings.search.name +
        '" placeholder="' +
        settings.customLanguage.searchPlaceholder +
        '">' +
        "</div>";
  ...
  prepareCollapsePannelButton() {
    if (this.settings.isMobile) {
      if (
        this.settings.dateRangeType ||
        typeof this.settings.checkBoxes[0] !== "undefined" ||
        typeof this.settings.multiSelect[0] !== "undefined" ||
        typeof this.settings.radioButtons[0] !== "undefined" ||
        typeof this.settings.singleDatepicker[0] !== "undefined" ||
        typeof this.settings.rangeDatepicker[0] !== "undefined" ||
        typeof this.settings.filterDropdown[0] !== "undefined"
      ) {
        var collapsePanelDom =
          "<div class='float-end filter-collapse-button'>\
                        <button title='" +
          AppLanugage.filters +
          "' class='dropdown-toggle btn btn-default mt0' data-bs-toggle='collapse' data-bs-target='#table-collapse-filter-" +
          this.randomId +
          "' aria-expanded='false'><i data-feather='sliders' class='icon-18'></i></button>\
                    </div>\
                    <div id='table-collapse-filter-" +
          this.randomId +
          "' class='navbar-collapse collapse w100p'></div>";

        this.$instanceWrapper
          .find(this.leftFilterSectionClsss)
          .append(collapsePanelDom);
      }
    }
  }
  prepareReloadButton() {
    ...
    if (it.settings.reloadSelector) {
      if (!$(it.settings.reloadSelector).length) {
        var reloadDom =
          ...
          it.settings.reloadSelector.slice(1) +
          '"><i data-feather="refresh-cw" class="icon-16"></i></button>' +
          "</div>";
        this.$instanceWrapper
          .find(this.leftFilterSectionClsss)
          .append(reloadDom); //bind refresh icon
      }
  ...
  prepareFilterFormShowButton() {
    if (this.settings.smartFilterIdentity) {
      ...
      var filterText =
        '<span class="add-filter-text ml5">' +
        AppLanugage.addNewFilter +
        "</span>";
      if (filters.length) {
        filterText = "";
      } else {
      ...
      var smartFilterDropdownDom =
        '<div class="filter-item-box">' +
        '<button class="btn btn-default show-filter-form-button" type="button"><i data-feather="plus" class="icon-16"></i>' +
        filterText +
        "</button>" +
        "</div>";
  ...
  prepareBookmarkFilterButtons() {
    if (this.settings.smartFilterIdentity) {
      ...
      it.$instanceWrapper
        ...
        .on("click", ".bookmarked-filter-button", function () {
          var data = $(this).data() || {},
            filterId = data.id;
          it.state = "new_filter";
          it.applySelectedFilter(filterId);
        ...
        if (filterItem.bookmark == "1") {
          ...
          if (filterItem.icon) {
            bookmarkButtonContent =
              '<i data-feather="' + filterItem.icon + '" class="icon-16"></i>';
          }

          var smartFilterDropdownDom =
            '<div class="filter-item-box bookmarked-filter-button-wrapper">' +
            '<button class="btn btn-default bookmarked-filter-button round" type="button" data-id="' +
            filterItem.id +
            '"  >' +
            bookmarkButtonContent +
            "</button>" +
  ...
  prepareSaveFilterButton() {
    if (this.settings.smartFilterIdentity) {
      ...
      var smartFilterCreateButon =
        '<div class="filter-item-box save-filter-box hide">' +
        '<button class="btn btn-default save-filter-button" data-act="ajax-modal" data-title="" ' +
        dataPostAttrs +
        '  type="button" data-action-url="' +
        actionUrl +
      ...
      this.$instanceWrapper
        .find(this.filterFormClass)
        .append(smartFilterCreateButon);
    }
  }
  prepareCancelFilterFormButton() {
    if (this.settings.smartFilterIdentity) {
      ...
      this.$instanceWrapper
        .find(this.filterFormClass)
        .append(smartFilterCancelButton);
  ...
  appendFilterDom(dom) {
    if (this.settings.smartFilterIdentity) {
      ...
      this.$instanceWrapper
        .find("#table-collapse-filter-" + this.randomId)
        .append(dom);
    } else {
  ...
  prepareDateRangePicker() {
    ...
    if (settings.dateRangeType) {
      var dateRangeFilterDom =
        '<div class="filter-item-box btn-group">' +
      ...
      if (settings.dateRangeType === "daily") {
        var initSingleDaySelectorText = function ($elector) {
          if (
            settings.filterParams.start_date ===
            moment().format(settings._inputDateFormat)
          ) {
            ...
            $elector.html(
              moment(settings.filterParams.start_date).format("Do MMMM YYYY")
            );
          }
        };
        // prepareDefaultDateRangeFilterParams();
        ...
        $dateRangeSelector.click(function () {
          var type = $(this).attr("data-act"),
            date = "";
          if (type === "next") {
            date = moment(settings.filterParams.start_date)
              .add(1, "days")
              .format(settings._inputDateFormat);
          } else if (type === "prev") {
            date = moment(settings.filterParams.start_date)
      ...
      if (settings.dateRangeType === "monthly") {
        var initMonthSelectorText = function ($elector) {
          $elector.html(
            moment(settings.filterParams.start_date).format("MMMM YYYY")
          );
        };
      ...
      if (settings.dateRangeType === "yearly") {
        var inityearSelectorText = function ($elector) {
          $elector.html(
            moment(settings.filterParams.start_date).format("YYYY")
          );
        };
        // prepareDefaultDateRangeFilterParams();
  ...
  prepareDropdownFilters() {
    ...
    if (typeof settings.filterDropdown[0] !== "undefined") {
      $.each(settings.filterDropdown, function (index, dropdown) {
        ...
        var selectDom =
          '<div class="filter-item-box">' + selectDomSelector + "</div>";
          ...
          setValue: function (value, newFilterParams) {
            ...
            if (dropdown.showHtml && !value) {
              if (
                selectHtmlData[0] &&
                !selectHtmlData[0].id &&
                selectHtmlData[0].text
              ) {
                $dropdown
                  .siblings(".select2-container")
                  .find(".select2-chosen")
                  .html(selectHtmlData[0].text);
              }
            }
    ...
    if (typeof settings.rangeDatepicker[0] !== "undefined") {
      $.each(settings.rangeDatepicker, function (index, datePicker) {
        var startDate = datePicker.startDate || {},
          ...
          startButtonText = startDate.value
            ? moment(startDate.value, settings._inputDateFormat).format(
                "Do MMMM YYYY"
              )
            : emptyText,
          endButtonText = endDate.value
            ? moment(endDate.value, settings._inputDateFormat).format(
                "Do MMMM YYYY"
              )
            : emptyText;
        ...
        if (datePicker.label) {
          devider =
            '<span class="input-group-addon custom-date-range-lable">' +
            datePicker.label +
            "</span>";
          ...
          if (datePicker.ranges) {
            ...
            $.each(datePicker.ranges, function (index, range) {
              if (defaultRanges[range]) {
                options +=
                  '<li><a href="#" class="dropdown-item list-group-item clickable" data-range="' +
                  range +
                  '">';
            ...
            if (options) {
              ...
              var rangeDropdownDom =
                ...
                "</div>";
              devider =
                '<span class="input-group-addon custom-date-range-dropdown clickable">' +
                rangeDropdownDom +
                "</span>";
            }
          }
        ...
        var selectDom =
          ...
          dateRangeClass +
          '">' +
          '<button class="btn btn-default form-control" name="' +
          startDate.name +
          '" data-date="' +
          startDate.value +
          '">' +
        ...
        var showButtonText = function () {
          ...
          $(inputs[0]).html(
            s_date
              ? moment(s_date, settings._inputDateFormat).format("Do MMMM YYYY")
              : emptyText
          );
          $(inputs[1]).html(
            e_date
              ? moment(e_date, settings._inputDateFormat).format("Do MMMM YYYY")
              : emptyText
          );
        };
        ...
        $datePicker
          ...
          .on("changeDate", function (e) {
            var date = moment(e.date, settings._inputDateFormat).format(
              settings._inputDateFormat
            );
          ...
          .on("show", function () {
            ...
            if (showClearButton) {
              ...
              if (!$(".datepicker-clear-selection").length) {
                $(".datepicker").append(
                  "<div class='datepicker-clear-selection p5 clickable text-center'>" +
                    AppLanugage.clear +
                    "</div>"
                );
    ...
    if (typeof settings.singleDatepicker[0] !== "undefined") {
      $.each(settings.singleDatepicker, function (index, datePicker) {
        ...
        if (!selectedText) {
          ...
          options =
            '<div class="list-group-item active" data-value="">' +
            selectedText +
            "</div>" +
            options;
        }
        ...
        //prepare DOM
        var customList =
          '<div class="datepicker-custom-list list-group mb0">' +
          options +
          "</div>";
        ...
        var $datePicker = $instanceWrapper.find(
            "[name='" + datePicker.name + "']"
          ),
          showCustomRange =
            typeof datePicker.options[1] === "undefined" ? false : true; //don't show custom range if options not > 1
        ...
        $datePicker
          ...
          .on("show", function () {
            ...
            if (showCustomRange) {
              $(
                ".datepicker-days, .datepicker-months, .datepicker-years, .datepicker-decades, .table-condensed"
              ).hide();
              ...
              if (!$(".datepicker-custom-list").length) {
                ...
                $(".datepicker .list-group-item").click(function () {
                  ...
                  if (value === "show-date-picker") {
                    $(
                      ".datepicker-custom-list, .datepicker-months, .datepicker-years, .datepicker-decades, .table-condensed"
                    ).hide();
                    ...
                    if (moment(value, settings._inputDateFormat).isValid()) {
                      value = moment(value, settings._inputDateFormat).format(
                        settings._inputDateFormat
                      );
                    }
          ...
          .on("changeDate", function (e) {
            $datePicker.html(
              getDatePickerText(
                moment(e.date, settings._inputDateFormat).format("Do MMMM YYYY")
              )
            );
            reloadDatePickerFilter(
              moment(e.date, settings._inputDateFormat).format(
                settings._inputDateFormat
              )
            );
          });
        ...
        it.filterElements[datePicker.name] = {
          setValue: function (value) {
            ...
            if (value && text) {
              ...
              $datePicker.html(
                getDatePickerText(
                  moment(value, settings._inputDateFormat).format(
                    "Do MMMM YYYY"
                  )
                )
              );
  ...
  prepareRadioFilter() {
    ...
    if (typeof settings.radioButtons[0] !== "undefined") {
      ...
      var $radioButtons = $instanceWrapper.find(
        "[data-act='radio'] input[type=radio]"
      );
      ...
      $.each(settings.multiSelect, function (index, select) {
        ...
        $.each(select.options, function (index, listOption) {
          ...
          if (
            (saveSelection &&
              selections &&
              selections.indexOf(listOption.value) > -1) ||
        ...
        multiSelect =
          "<div class='dropdown-menu'><ul class='list-group' data-act='multiselect'>" +
          multiSelect +
          "</ul></div>";
        ...
        var multiSelectDom =
          ...
          '<button class="' +
          multiSelectClass +
          ' btn btn-default dropdown-toggle caret " type="button" data-bs-toggle="dropdown" aria-expanded="true">' +
          select.text +
          " </button>" +
          multiSelect +
          "</span>" +
        ...
        var $multiSelect = $instanceWrapper.find(
  ...
  prepareDependentFilter(
    ...
    $.each(filterDropdown, function (index, option) {
      ...
      if (
        option.dependency &&
        option.dependency.length &&
        option.dependency.indexOf(filterName) !== -1
      ) {
        var $dependencySelector = $instanceWrapper.find(
        ...
        if (
          ((option.selfDependency && !filterValue) || filterValue) &&
          option.dataSource
        ) {
          $.ajax({
        ...
        if (
          ...
          filterParams[dependentFilterName] =
            newFilterParams[dependentFilterName];
...
if (typeof TableTools != "undefined") {
  TableTools.DEFAULTS.sSwfPath =
    ...
    "js/datatable/TableTools/swf/copy_csv_xls_pdf.swf";
}
...
(function ($) {
  ...
  $.fn.appTable = function (options) {
    ...
    if (AppHelper.settings.disableResponsiveDataTable === "1") {
      ...
      AppHelper.settings.disableResponsiveDataTableForMobile !== "1" &&
    ...
    if (settings.newData) {
      ...
      if (settings.dataId) {
        ...
        if ($row.length) {
          ...
          table
            .api()
            .row(table.api().row($row.closest("tr")).index())
    ...
    var _prepareFooter = function (settings, page, lable) {
      ...
      $.each(settings.columns, function (index, column) {
        ...
        if (
          settings.summation[0] &&
          settings.summation[0].column - 1 === index
        ) {
          thLable = lable;
    ...
    $.each(settings.columns, function (index, column) {
      ...
      if (settings.serverSide) {
        if (column.order_by) {
          ...
          !column.order_by &&
          column.iDataSort &&
          settings.columns[column.iDataSort].order_by
        ) {
          orderable = true;
        }
      } else {
    ...
    settings._exportable =
      settings.xlsColumns.length +
      settings.pdfColumns.length +
      settings.printColumns.length;
    ...
    var datatableOptions = {
      ...
      ajax: {
        ...
        data: function (postData) {
          var order_by_index =
              postData.order && postData.order[0]
                ? postData.order[0].column
                : "",
            order_dir =
              postData.order && postData.order[0] ? postData.order[0].dir : "",
          ...
          return $.extend(
            {
              order_by: settings.columns[order_by_index]
                ? settings.columns[order_by_index].order_by
                : "",
              order_dir: order_dir,
              search_by: search,
              skip: postData.start,
      ...
      footerCallback: function (row, data, start, end, display) {
        ...
        if (settings.summation) {
          ...
          $.each(settings.summation, function (index, option) {
            ...
            var pageTotal = calculateDatatableTotal(
              instance,
              option.column,
              function (currentValue) {
                ...
                if (currentValue && !currentValue.startsWith("<b>")) {
                  if (option.dataType === "currency") {
                    if (option.dynamicSymbol) {
                      //find out currency symbol
                      var x = currentValue;
                      option.currencySymbol = x.replace(/[0-9.,-]/g, "");
                    }

                    return unformatCurrency(
                      currentValue,
            ...
            if (pagTotalTitle) {
              pageTotalContent +=
                ...
                $(pagTotalTitle).html() +
                "</div><div class='box-content text-right'>" +
                pageTotal +
                "</div></div>";
            }

            $(instance)
              .find("[data-current-page=" + option.column + "]")
              .html(pageTotal);

            // total value of all pages
            if (pageInfo.pages > 1) {
              $(instance).find("[data-section='all_pages']").show();
              var total = calculateDatatableTotal(
                instance,
                option.column,
                function (currentValue) {
                  //if we get <b> tag, we'll assume that is a group total. ignore the value
              ...
              $(instance)
                .find("[data-all-page=" + option.column + "]")
                .html(total);
            } else {
          ...
          if (settings.isMobile) {
            if (pageTotalContent) {
              summationContent +=
                "<div class='box'><div class='box-content strong'>" +
                AppLanugage.total +
                "</div></div>" +
                pageTotalContent;
            }
      ...
      if (settings.xlsColumns.length) {
        ...
        datatableButtons.push({
          ...
          customize: function (xls) {
            ...
            if (
              $instance.find("[data-section='all_pages']") &&
              $instance.find("[data-section='all_pages']").css("display") !==
                "none"
            ) {
              ...
              var thisRowNumber =
                parseInt($sheetSelector.find("row:last-child").attr("r")) + 1;
              ...
              sheet.childNodes[0].childNodes[1].innerHTML =
                sheet.childNodes[0].childNodes[1].innerHTML + rowDom;
            }
          },
        });
...
=======
        ...
        datatableButtons.push({
          ...
          customize: function (win) {
            ...
            if (
              ...
              var totalOfAllPagesClone = $instance
                .find("[data-section='all_pages']")
                .clone();
              $(win.document.body).find("tfoot").append(totalOfAllPagesClone);
            }
          },
    ...
    //set custom toolbar
    if (!settings.hideTools) {
      datatableOptions.sDom =
        "<'filter-section-container' <'filter-section-flex-row' <'filter-section-left'> <'filter-section-right' " +
        sDomExport +
        " <'filter-item-box' f> > > " +
        filterFormDom +
        " r>t<'datatable-tools clearfix row'<'col-md-3 pl15'<'summation-section'> li><'col-md-9 pr15'p>>";
    }
    ...
    datatableOptions.drawCallback = function () {
      if (settings.serverSide) {
        $instance
          ...
          .val(
            settings.filterParams && settings.filterParams.search_by
              ? settings.filterParams.search_by
              : ""
          );
      }
    ...
    if (settings.columnShowHideOption) {
      ...
      //prepare a popover
      var popover =
        '<div class="filter-item-box"><button class="btn btn-default column-show-hide-popover" data-container="body" data-bs-toggle="popover" data-placement="bottom"><i data-feather="columns" class="icon-16"></i></button></div>';
      $instanceWrapper.find(".filter-section-left").append(popover);
          ...
          $.each(settings.columns, function (index, column) {
            ...
            if (column.visible !== false) {
              ...
              if (!tableColumn.visible()) {
                columnHiddenClass = "active";
                eyeOnOffIcon =
                  "<i data-feather='eye-off' class='icon-16 mr10'></i>";
              }
              ...
              tableColumns +=
                "<li class='" +
                columnHiddenClass +
                " list-group-item clickable toggle-table-column' data-column='" +
                index +
                "'>" +
                eyeOnOffIcon +
                column.title +
    ...
    if (settings.tableRefreshButton) {
      ...
      var refreshButton =
        '<div class="filter-item-box float-start "><button class="btn btn-default at-table-refresh-button ml15"><i data-feather="refresh-cw" class="icon-16"></i></button></div>';
      $instanceWrapper.find(".filter-section-left").append(refreshButton);

      $instanceWrapper
        .find(".at-table-refresh-button")
        .on("click", function () {
          $instance.appTable({
            reload: true,
    ...
    if (!$("body").hasClass("destroy-popover")) {
      ...
      $(".destroy-popover").on("click", function (e) {
        if (
          $(e.target).closest("button").attr("data-bs-toggle") !== "popover" &&
    ...
    if (settings.serverSide) {
      ...
      $searchBox.unbind().bind(
        "input",
        delayAction(function (e) {
          settings.filterParams.search_by = $(this).val();
          $instance.appTable({
            reload: true,
            filterParams: settings.filterParams,
      ...
      $("body").on(
        "click",
        "#" + $instance.get(0).id + " .badge.clickable",
        function () {
          settings.filterParams.search_by = $(this).text();
          $instance.appTable({
            reload: true,
      ...
      $("body").on(
        "click",
        "#" + $instance.get(0).id + " .filter-sub-task-button",
        function () {
          settings.filterParams.search_by = $(this).attr("main-task-id");
          $instance.appTable({
            reload: true,
      ...
      $("body").on(
        "click",
        "#" + $instance.get(0).id + " .remove-filter-button",
        function () {
          settings.filterParams.search_by = "";
          $instance.appTable({
            reload: true,
    ...
    var undoHandler = function (eventData) {
      $(
        '<a class="undo-delete" href="javascript:;"><strong>' +
          AppLanugage.undo +
          "</strong></a>"
      ).insertAfter($(eventData.alertSelector).find(".app-alert-message"));
      $(eventData.alertSelector)
        .find(".undo-delete")
        .bind("click", function () {
          $(eventData.alertSelector).remove();
          appLoader.show();
    ...
    $("body")
      .find($instance)
      .on("click", "a[data-action=delete]", appTableSimpleDeleteHandler);
    $("body")
      .find($instance)
      .on(
        "click",
        "a[data-action=delete-confirmation]",
        appTableDeleteConfirmationHandler
      );
    $("body")
      .find($instance)
      .on("click", "[data-action=update]", updateHandler);
    ...
    $.fn.dataTableExt.oApi.fnUpdateRow = function (
      oSettings,
      data,
      page,
      renderBeforePageChange
    ) {
      //oSettings is not any parameter, we'll get it automatically.
    ...
    return (function () {
      ...
      function show(options) {
        ...
        if (!$template.length) {
          ...
          if ($container.length) {
            $container.append(
              '<div id="app-loader" class="app-loader" style="z-index:' +
                this._settings.zIndex +
                ";" +
                this._settings.css +
...
=======
        var controlIcon =
          "<span class='expand hidden-xs'><i data-feather='maximize-2' class='icon-16'></i></span>";

        if (settings.sidebar === false || isMobile()) {
          sidebar = "";
          controlIcon =
            "<div class='app-modal-close app-modal-fixed-close-button'><span>&times;</span></div>";
        }
        ...
        setTimeout(function () {
          var windowHeight = $(window).height() - 60;
          if (
            $(".app-modal-content-area").prop("scrollHeight") > windowHeight
          ) {
        ...
        appModalXhr = $.ajax({
          ...
          success: function (response) {
            var $content = $(response);
            $(".app-modal-content-area").html(
              $content.find(".app-modal-content").html()
            );
            $(".app-modal-sidebar-area").html(
              $content.find(".app-modal-sidebar").html()
            );
            $content.remove();
            $(".app-modal").removeClass("loading");
            appLoader.hide();
...
=======
  $.fn.appFilters = function (options) {
    ...
    if (settings.reload) {
      ...
      if (instance.data("beforeRelaodCallback")) {
        instance.data("beforeRelaodCallback")(
          instance,
          instanceSettings.filterParams
        );
      }
      ...
      if (instance.data("afterRelaodCallback")) {
        instance.data("afterRelaodCallback")(
          instance,
          instanceSettings.filterParams
        );
      }
...
calculateDatatableTotal = function (
  instance,
  columnNumber,
  valueModifier,
  currentPage
  ...
  return api
    .column(columnNumber, columnOption)
    .data()
    .reduce(function (previousValue, currentValue, test, test2) {
      if (valueModifier) {
        return previousValue + valueModifier(currentValue);
  ...
  $.fn.appMultiSelect = function (options) {
    ...
    this.each(function () {
      ...
      var dom =
        '<div class="">' +
        '<span class="dropdown inline-block filter-multi-select">' +
        '<button class="btn btn-default dropdown-toggle caret " type="button" data-bs-toggle="dropdown" aria-expanded="true">' +
        settings.text +
        " </button>" +
        multiSelect +
        "</span>" +
...
=======
  $.fn.appModifier = function (options) {
    ...
    if (settings.showbuttons) {
      buttonDom =
        "<div class='custom-popover-button-area mt10 clearfix row'>\n\
                            <div id='custom-popover-submit-btn-" +
        tempId +
        "' class='col-md-6 pr5'><button class='btn btn-primary btn-sm w100p'><i data-feather='check' class='icon-16'></i></button></div>\n\
                            <div class='col-md-6 pl5 custom-popover-close-btn'><button class='btn btn-default btn-sm w100p'><i data-feather='x' class='icon-16'></i></button></div>\n\
                        </div>";
    }
    ...
    if (settings.actionType === "select2") {
      containerDom =
        ...
        "' value='" +
        settings.value +
        "' type='text' class='form-control popover-tempId' /> " +
        buttonDom;
    } else if (settings.actionType === "date") {
      var dateFormat = getJsDateFormat();
      var dateArray = settings.value.split("-"),
      ...
      containerDom =
        "<div style='height: 240px;' id='" +
        tempId +
        "'  data-date='" +
        dateValue +
        "' data-date-format='" +
        dateFormat +
        "' class='popover-tempId'></div>"; //set height first for right popover position
    } else if (settings.actionType === "time") {
      containerDom =
        ...
        "'  value='" +
        settings.value +
        "' /><div id='popover-timepicker-container-" +
        tempId +
        "' ></div>" +
        buttonDom;
    }
    ...
    //create popover dom
    var popoverDom =
      "<div class='app-popover' style='top: " +
      topOffset +
      "px; left: " +
      leftOffset +
      "px'>\n\
    ...
    if (settings.actionType === "select2") {
      ...
      if (settings.showbuttons) {
        ...
        $("#" + tempId)
          .select2(settings.select2Option)
      ...
      var appendWidgetTo = "#popover-timepicker-container-" + tempId;
      var showMeridian =
        AppHelper.settings.timeFormat == "24_hours" ? false : true;
    ...
    $("body")
      ...
      .each(function () {
        ...
        if (right < 0) {
          ...
          var right =
            $(window).width() -
            ($instance.offset().left + ($instance.outerWidth() / 2) * 1);
          $(this)
            .find(".app-popover-arrow")
            .css({ left: "unset", right: right });
        }
      });
    ...
    function initAjaxAction($instance, value, settings, changedText) {
      var popoverContentHeight = $inputField
        .closest(".app-popover-body")
        .height();
      var popoverContentWidth = $inputField
        .closest(".app-popover-body")
        .width();
      $inputField
        .closest(".app-popover-body")
        ...
        .removeClass("hide")
        .css({ height: popoverContentHeight, width: popoverContentWidth });
      $inputField
        .closest(".app-popover-body")
        .find(".custom-popover-button-area")
        .addClass("hide");
      $inputField.addClass("hide");
      $timepickerContainer.addClass("hide");
...
=======
  $.fn.appConfirmation = function (options) {
    ...
    //create popover dom
    var popoverDom =
      "<div class='app-popover' style='bottom: " +
      bottomOffset +
      "px; left: " +
      leftOffset +
      "px'>\n\
      ...
      "</div>\n\
                                    <div class='app-popover-body pt0'>\n\
                                        <div class='custom-popover-button-area mt15 clearfix row'>\n\
                                            <div class='col-md-6 pr5'><button class='btn btn-danger btn-sm w100p confirmation-confirm-button'><i data-feather='check' class='icon-16'></i> " +
      settings.btnConfirmLabel +
      "</button></div>\n\
                                            <div class='col-md-6 pl5'><button class='btn btn-default btn-sm w100p confirmation-cancel-button'><i data-feather='x' class='icon-16'></i> " +
      settings.btnCancelLabel +
      "</button></div>\n\
>>>>>>> d1c9620768f386677ac1d351c2c7dcec4ee00ee3
                                        </div>\n\
                                    </div>\n\
