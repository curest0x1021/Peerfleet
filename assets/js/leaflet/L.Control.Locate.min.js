...
!(function (t, i) {
  ...
  var i = t.Control.extend({
    options: {
      ...
      circlePadding: [0, 0],
      metric: !0,
      createButtonCallback: function (i, s) {
        var o = t.DomUtil.create("a", "leaflet-bar-part leaflet-bar-part-single", i);
        return (
          (o.title = s.strings.title),
          {
            link: o,
            icon: t.DomUtil.create(s.iconElementTag, s.icon, o),
          }
        );
      },
    ...
    initialize: function (i) {
      for (var s in i) "object" == typeof this.options[s] ? t.extend(this.options[s], i[s]) : (this.options[s] = i[s]);
    ...
    onAdd: function (i) {
      var s = t.DomUtil.create("div", "leaflet-control-locate leaflet-bar leaflet-control");
      (this._layer = this.options.layer || new t.LayerGroup()),
        this._layer.addTo(i),
        (this._event = void 0),
        (this._prevBounds = null);
      var o = this.options.createButtonCallback(s, this.options);
      return (
        (this._link = o.link),
        (this._icon = o.icon),
          ...
          case "stop":
            this.stop(),
              this.options.returnToPrevBounds &&
                (this.options.flyTo ? this._map.flyToBounds : this._map.fitBounds).bind(this._map)(this._prevBounds);
        }
    ...
    _drawMarker: function () {
      ...
      if (this.options.drawCircle) {
        ...
        this._circle
          ? this._circle.setLatLng(s).setRadius(i).setStyle(o)
          : (this._circle = t.circle(s, i, o).addTo(this._layer));
      }
      var e, n;
      if (
        (this.options.metric
          ? ((e = i.toFixed(0)), (n = this.options.strings.metersUnit))
          : ((e = (3.2808399 * i).toFixed(0)), (n = this.options.strings.feetUnit)),
        this.options.drawMarker)
      ) {
        var a = this._isFollowing() ? this.options.followMarkerStyle : this.options.markerStyle;
        this._marker
          ? (this._marker.setLatLng(s), this._marker.setStyle && this._marker.setStyle(a))
          : (this._marker = new this.options.markerClass(s, a).addTo(this._layer));
      }
    ...
    _isFollowing: function () {
      return (
        !!this._active &&
        ("always" === this.options.setView || ("untilPan" === this.options.setView ? !this._userPanned : void 0))
      );
    },
    ...
    _setClasses: function (i) {
      "requesting" == i
        ? (t.DomUtil.removeClasses(this._container, "active following"),
          t.DomUtil.addClasses(this._container, "requesting"),
          t.DomUtil.removeClasses(this._icon, this.options.icon),
          t.DomUtil.addClasses(this._icon, this.options.iconLoading))
        : "active" == i
        ? (t.DomUtil.removeClasses(this._container, "requesting following"),
          t.DomUtil.addClasses(this._container, "active"),
          t.DomUtil.removeClasses(this._icon, this.options.iconLoading),
          t.DomUtil.addClasses(this._icon, this.options.icon))
        : "following" == i &&
          (t.DomUtil.removeClasses(this._container, "requesting"),
          t.DomUtil.addClasses(this._container, "active following"),
          t.DomUtil.removeClasses(this._icon, this.options.iconLoading),
          t.DomUtil.addClasses(this._icon, this.options.icon));
    },
    ...
    (function () {
      var i = function (i, s, o) {
        (o = o.split(" ")).forEach(function (o) {
          t.DomUtil[i].call(this, s, o);
        });
      };
      (t.DomUtil.addClasses = function (t, s) {
        i("addClass", t, s);
      }),
        (t.DomUtil.removeClasses = function (t, s) {
          i("removeClass", t, s);
        });
    })(),
    i
