...
class Shackles_loadtest_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $sql = "SELECT $shackles_table.internal_id, $shackles_table.id as shackle_id, k.initial_test_date, c.id, c.test_date, c.tested_by, c.location, c.passed, c.remarks
                FROM $shackles_table
                LEFT JOIN (
                    SELECT shackle_id, test_date as initial_test_date FROM $loadtest_table WHERE initial_test = 1
                    ) k ON $shackles_table.id = k.shackle_id
                LEFT JOIN (
    ...
    function is_allow_initial_test($shackle_id, $id = null) {
        $loadtest_table = $this->db->prefixTable("shackles_loadtest");

        $sql = "SELECT * FROM $loadtest_table WHERE shackle_id=$shackle_id AND initial_test=1";
