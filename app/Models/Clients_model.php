...
class Clients_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        CONCAT(t.first_name, ' ', t.last_name) AS primary_contact, t.id AS primary_contact_id, t.image AS tech_avatar,
                owner_details.owner_name, owner_details.owner_avatar, owner_details.owner_email
        FROM $clients_table
        ...
        LEFT JOIN (SELECT $users_table.id, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS owner_name, $users_table.image AS owner_avatar, $users_table.email AS owner_email FROM $users_table WHERE $users_table.deleted=0 AND $users_table.is_admin=0 ) AS owner_details ON owner_details.id=$clients_table.owner_id
