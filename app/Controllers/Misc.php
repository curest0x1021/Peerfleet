...
class Misc extends Security_Controller
{
    ...
    private function _make_row($data) {
        ...
        if ($this->can_access_own_client($data->client_id)) {
            $name = anchor(get_uri("misc/view/" . $data->client_id . "/" . $data->require_loadtests . "/" . $data->require_inspections), $data->name);
        }
    ...
    private function _loadtest_make_row($data, $client_id, $showInternalId = true) {
        ...
        if ($this->can_access_own_client($client_id)) {
            if ($showInternalId) {
                $internal_id = anchor(get_uri("misc/loadtest_detail_view/" . $data->misc_id), $internal_id, array("class" => "edit", "title" => app_lang('misc')));
                $action = modal_anchor(get_uri("misc/loadtest_modal_form/" . $data->misc_id), "<i data-feather='edit' class='icon-16'></i>", array("class" => "add", "title" => app_lang('add_item'), "data-post-force_refresh" => true));
            } else {
    ...
    private function _inspection_make_row($data, $client_id, $showInternalId = true) {
        ...
        if ($this->can_access_own_client($client_id)) {
            if ($showInternalId) {
                $internal_id = anchor(get_uri("misc/inspection_detail_view/" . $data->misc_id), $internal_id, array("class" => "edit", "title" => app_lang('misc')));
                $action = modal_anchor(get_uri("misc/inspection_modal_form/" . $data->misc_id), "<i data-feather='edit' class='icon-16'></i>", array("class" => "add", "title" => app_lang('add_item'), "data-post-force_refresh" => true));
            } else {
