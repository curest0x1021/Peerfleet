...
class Grommets extends Security_Controller
{
    ...
    private function _make_row($data) {
        ...
        if ($this->can_access_own_client($data->client_id)) {
            $name = anchor(get_uri("grommets/main_view/" . $data->client_id), $data->name);
        }
    ...
    function main_view($client_id) {
        ...
        return $this->template->rander("grommets/main/index", $view_data);
    }
    ...
    function main_modal_form($client_id) {
        ...
        return $this->template->view("grommets/main/modal_form", $view_data);
    }
    ...
    function save_main() {
        ...
        if ($main_row) {
            $m_save_id = $this->Grommets_main_model->ci_save($main_data, $main_row->id);
        } else {
        ...
        $save_id = $this->Grommets_model->ci_save($data);
        if ($save_id) {
    ...
    function main_list_data($client_id) {
        $list_data = [];
        if ($client_id) {
            $list_data = $this->Grommets_main_model->get_details($client_id);
        }
    ...
    private function _main_make_row($data, $client_id) {
        ...
        if ($this->can_access_own_client($client_id)) {
            $item = anchor(get_uri("grommets/view/" . $client_id . "/" . $data->id), $item);
        }
    ...
    function view($client_id, $main_id) {
        if ($client_id && $main_id) {
            ...
            $view_data['can_edit_items'] = $this->can_access_own_client($client_id);
            $view_data['vessel'] = $this->Clients_model->get_one($client_id);
            $view_data['main_info'] = $this->Grommets_main_model->get_one($main_id);

            $info = $this->Grommets_model->get_warnning_info($client_id, $main_id);
    ...
    function save_info() {
        ...
        $target_path = getcwd() . "/" . get_general_file_path("grommets", $data["client_id"]);
    ...
    function info_list_data($client_id, $main_id) {
        $list_data = $this->Grommets_model->get_grommets_details(array("client_id" => $client_id, "main_id" => $main_id))->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _info_make_row($data, $is_certificate = false) {
        ...
        $action = "";
        if ($this->can_access_own_client($data->client_id)) {
            $internal_id = $is_certificate ? $data->internal_id : modal_anchor(get_uri("grommets/info_detail_view/" . $data->id), $data->internal_id, array("class" => "edit", "title" => app_lang('grommets_wire_slings'), "data-post-id" => $data->id));
            $action = modal_anchor(get_uri("grommets/info_modal_form/" . $data->client_id . "/" . $data->main_id), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_item'), "data-post-id" => $data->id))
        ...
        if (is_array($files)) {
            foreach ($files as $key => $file) {
                ...
                $files_str .= anchor(get_uri("grommets/download_certificate_file/" . $data->id . "/" .$key), remove_file_prefix($file["file_name"]));
            }
        }
    ...
    function info_modal_form($client_id, $main_id) {
        ...
        $view_data["client_id"] = $client_id;
        $model_info = $this->Grommets_model->get_one($id);
        $main_info = $this->Grommets_main_model->get_one($main_id);
        if (!$id) {
            $next_internal_id = $this->Grommets_model->get_next_internal_id($client_id, $main_id);
    ...
    function loadtest_list_data($client_id, $main_id) {
        $list_data = $this->Grommets_loadtest_model->get_details(array("client_id" => $client_id, "main_id" => $main_id))->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    private function _loadtest_make_row($data, $client_id, $showInternalId = true, $last_date = true) {
        ...
        if ($this->can_access_own_client($client_id)) {
            if ($showInternalId) {
                $internal_id = anchor(get_uri("grommets/loadtest_detail_view/" . $data->grommet_id), $internal_id, array("title" => $data->internal_id));
                $action = modal_anchor(get_uri("grommets/loadtest_modal_form/" . $data->grommet_id), "<i data-feather='plus-circle' class='icon-16'></i>", array("class" => "add", "title" => app_lang('add_item'), "data-post-force_refresh" => true));
            } else {
    ...
    // Load grommets certificate tab
    function certificate_tab($client_id, $main_id) {
        if ($client_id && $main_id) {
            $view_data['client_id'] = $client_id;
            $view_data['main_id'] = $main_id;
            $view_data['can_edit_items'] = $this->can_access_own_client($client_id);
            return $this->template->view("grommets/certificate/index", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function inspection_list_data($client_id, $main_id) {
        $list_data = $this->Grommets_inspection_model->get_details(array("client_id" => $client_id, "main_id" => $main_id))->getResult();
        $result = array();
        foreach ($list_data as $data) {
    ...
    function certificate_list_data($client_id, $main_id) {
        $list = $this->Grommets_model->get_grommets_details(array("client_id" => $client_id, "main_id" => $main_id))->getResult();
        $result = [];
    ...
    function download_certificate_file($id, $key) {
        ...
        return $this->download_app_files(get_general_file_path("grommets", $client_id), $file_data);
    }
    ...
    private function _inspection_make_row($data, $client_id, $showInternalId = true, $last_date = true) {
        ...
        if ($this->can_access_own_client($client_id)) {
            if ($showInternalId) {
                $internal_id = anchor(get_uri("grommets/inspection_detail_view/" . $data->grommet_id), $internal_id, array("title" => $data->internal_id));
                $action = modal_anchor(get_uri("grommets/inspection_modal_form/" . $data->grommet_id), "<i data-feather='plus-circle' class='icon-16'></i>", array("class" => "add", "title" => app_lang('add_item'), "data-post-force_refresh" => true));
            } else {
