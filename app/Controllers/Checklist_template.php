...
class Checklist_template extends Security_Controller {
    ...
    function import_modal_form() {
        return $this->template->view("checklist_template/import_modal_form");
    }

    function download_sample_excel_file() {
        return $this->download_app_files(get_setting("system_file_path"), serialize(array(array("file_name" => "import-checklist_template-sample.xlsx"))));
    }
    ...
    function save_from_excel_file() {
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            //check duplicate checklist_template name, if found then show an error message
            if ($this->Equipments_model->is_duplicate_equipment_name($import_data["title"])) {
                continue;
            }

            //save leave data
            $leave_save_id = $this->Checklist_template_model->ci_save($import_data);
            if (!$leave_save_id) {
                continue;
            }
        }
