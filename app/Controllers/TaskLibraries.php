...
class TaskLibraries extends Security_Controller {
    ...
    private function _get_task_related_dropdowns($context = "", $context_id = 0, $return_empty_context = false) {
        ...
        if ($context === "client" && !$return_empty_context) {
            ...
            foreach ($clients as $client) {
                $clients_dropdown[] = array("id" => $client->id, "text" => $client->charter_name);
            }
        }
    ...
    function save() {
        ...
        $data = array(
            ...
            "dependencies" => $this->request->getPost("dependencies"),
        ...
        $save_id = $this->TaskLibrary_model->ci_save($data, $id);
        if ($save_id) {
            ...
            return redirect()->to("/task_libraries"."/".$save_id."/edit");
        } else {
    ...
    private function _row_data($id) {
        ...
        $data = $this->TaskLibrary_model->get_details($options)->getRow();
    ...
    private function _prepare_task_data($data_row, $allowed_headers) {
        ...
        foreach ($data_row as $row_data_key => $row_data_value) { //row values
            ...
            // }
            // else if ($header_key_value == "status") {
            //     $task_data["status"] = $this->$row_data_value;
            // }
    ...
    function save_task_from_excel_file() {
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            //save task data
            unset($task_data['labels']);
            unset($task_data['status_id']);
            unset($task_data['priority_id']);
            unset($task_data['start_date']);
            unset($task_data['deadline']);
            $task_save_id = $this->TaskLibrary_model->ci_save($task_data);
            $sort = $task_save_id;
