...
class Wire_type extends Security_Controller {
    ...
    function index() {
        return $this->template->rander('wire_type/index');
    }

    function modal_form() {
        $view_data["model_info"] = $this->Wire_type_model->get_one($this->request->getPost("id"));
        $view_data["label_column"] = "col-md-3";
        $view_data["field_column"] = "col-md-9";
        return $this->template->view('wire_type/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        if ($this->Wire_type_model->is_duplicate_wire_type_name($data["name"], $id)) {
            echo json_encode(array("success" => false, 'message' => app_lang("already_exists_wire_type_name")));
            exit();
        }

        $save_id = $this->Wire_type_model->ci_save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        $id = $this->request->getPost("id");
        if ($this->Wire_type_model->delete($id)) {
    ...
    function list_data() {
        $list_data = $this->Wire_type_model->get_all_where(array("deleted" => 0))->getResult();
        $result_data = [];
    ...
    private function _make_row($data) {
        $action = modal_anchor(get_uri("wire_type/modal_form"), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_equipment'), "data-post-id" => $data->id))
                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_equipment'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("wire_type/delete"), "data-action" => "delete-confirmation"));
    ...
    function import_modal_form() {
        return $this->template->view("wire_type/import_modal_form");
    }

    function download_sample_excel_file() {
        return $this->download_app_files(get_setting("system_file_path"), serialize(array(array("file_name" => "import-wire_type-sample.xlsx"))));
    }
...
/* End of file wire_type.php */
/* Location: ./app/controllers/wire_type.php */