    ...
    <div class="tab-title clearfix">
        <?php if ($can_edit_items) { ?>
            <div class="title-button-group">
                ...
                <?php echo modal_anchor(get_uri("consumables/import_modal_form/chemicals"), "<i data-feather='upload' class='icon-16'></i> " . app_lang('import_items'), array("class" => "btn btn-default", "title" => app_lang('import_items'))); ?>
                <?php echo modal_anchor(get_uri("consumables/chemicals_modal_form"), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_item'), array("class" => "btn btn-default", "title" => app_lang('add_item'))); ?>
            </div>
        <?php } ?>
    </div>
    <div class="table-responsive">
        <table id="chemical-table" class="display" cellspacing="0" width="100%">
        </table>
    </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#chemical-table").appTable({
            source: '<?php echo_uri("consumables/chemicals_list_data") ?>',
            checkBoxes: [
        ...
        $('body').on('click', '[data-act=update-chemical-critical-checkbox]', function () {
            $(this).find("span").removeClass("checkbox-checked");
            $(this).find("span").addClass("inline-loader");
            $.ajax({
                url: '<?php echo_uri("consumables/save_chemical_critical") ?>/' + $(this).attr('data-id'),
                type: 'POST',
                dataType: 'json',
                data: {value: $(this).attr('data-value')},
                success: function (response) {
                    if (response.success) {
                        $("#chemical-table").appTable({newData: response.data, dataId: response.id});
                    }
                }
            });
        });
