<div class="card">
    <div class="tab-title clearfix">
        <h4><?php echo app_lang('shackles_info'); ?></h4>
        <?php if ($can_edit_items) { ?>
            <div class="title-button-group">
                ...
                <?php echo modal_anchor(get_uri("shackles/info_modal_form/" . $client_id. "/" . $main_id), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_item'), array("class" => "btn btn-default", "title" => app_lang('add_item'))); ?>
            </div>
        <?php } ?>
    </div>
...
<script type="text/javascript">
    loadShacklesTable = function(selector) {
        $(selector).appTable({
            source: '<?php echo_uri("shackles/info_list_data/" . $client_id. "/" . $main_id) ?>',
            columns: [
