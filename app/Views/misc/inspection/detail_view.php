                    ...
                    <ul id="loadtest-tabs" data-bs-toggle="ajax-tab" class="nav nav-tabs bg-white title" role="tablist">
                        ...
                        <div class="tab-title clearfix no-border">
                            <div class="title-button-group">
                                <?php echo modal_anchor(get_uri("misc/inspection_modal_form/" . $misc->id), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_item'), array("class" => "btn btn-default", "title" => app_lang('add_item'), "data-post-force_refresh" => false)); ?>
                            </div>
                        </div>
                    </ul>
                    <div class="card">
...
<script type="text/javascript">
    loadInspectionDetailTable = function(selector) {
        $(selector).appTable({
            source: '<?php echo_uri("misc/inspection_detail_list_data/" . $misc->client_id . "/" . $misc->id) ?>',
            order: [[2, "desc"]],
            columns: [
