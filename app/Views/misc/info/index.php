    ...
    <div class="tab-title clearfix">
        ...
        <?php if ($can_edit_items) { ?>
            <div class="title-button-group">
                ...
                <?php echo modal_anchor(get_uri("misc/info_modal_form/" . $client_id), "<i data-feather='plus-circle' class='icon-16'></i> " . app_lang('add_item'), array("class" => "btn btn-default", "title" => app_lang('add_item'))); ?>
            </div>
        <?php } ?>
    </div>
...
<script type="text/javascript">
    loadMiscTable = function(selector) {
        $(selector).appTable({
            source: '<?php echo_uri("misc/info_list_data/" . $client_id) ?>',
            columns: [
