<?php echo form_open(get_uri("warehouses/save_chemical"), array("id" => "chemicals-form", "class" => "general-form", "role" => "form")); ?>
<div class="modal-body clearfix">
    <div class="container-fluid">
        ...
        <div class="form-group">
            <div class="row">
                <label for="chemical_id" class="<?php echo $label_column; ?>"><?php echo app_lang('item'); ?></label>
                <div class="<?php echo $field_column; ?>">
                    <?php
                    echo form_dropdown("chemical_id", $items_dropdown, array($model_info->chemical_id), "class='select2 validate-hidden' id='chemical_id' data-rule-required='true' data-msg-required='" . app_lang('field_required') . "'");
                    ?>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        chemicals = JSON.parse('<?php echo json_encode($chemicals); ?>');
        const modelInfo = JSON.parse('<?php echo json_encode($model_info);?>');

        $("#chemicals-form").bind("submit", function() {
            $("#chemical_id").prop("disabled", false);
        });

        $("#chemicals-form").appForm({
            onSuccess: function(result) {
                if (result.min_stock_reached) {
                    ...
                    // $("#chemicals-table").appTable({newData: result.data, dataId: result.id});
                }
                setTimeout(() => {
                    window.location.reload();
                }, 500);
        ...
        $('#chemical_id').select2();
        $("#chemical_id").change((e) => {
            generateInfo(e.target.value);
        });

        if (modelInfo.id) {
            generateInfo(modelInfo.chemical_id);
            $("#chemical_id").prop("disabled", true);
        }

    });

    function generateInfo(id) {
        const index = chemicals.findIndex((k) => k.id === id);
            if (index > -1) {
